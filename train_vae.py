...
def main():
    # create direc
    VAE_CKPT = 'vae_checkpoints'
    CHECK_IMG_PATH = 'vae_checkimages'
    
    if not os.path.exists(VAE_CKPT):
        os.makedirs(VAE_CKPT)
        ...
        for i_split in range(args.num_splitted):
            for i_batch, imgs_list in enumerate(loader):
                ...
                if i_batch % args.save_freq == 0:
                    ...
                    torch.save(model.state_dict(), f"./{VAE_CKPT}/model.pt")
                    torch.save(model.encoder.state_dict(), f"./{VAE_CKPT}/encoder.pt")
